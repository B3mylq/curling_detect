...
void ClusterCallback(const sensor_msgs::PointCloud2::ConstPtr& point_msg)
{
    if(spin_count > 0){
        return;
    }
    ...
    writer.write<pcl::PointXYZ> ("/home/b3mylq/ROS_ws/pcl_ws/src/depth_make/data/hex.pcd", *cloud, false);
    ...
    pcl::ExtractIndices<pcl::PointXYZ> extract_dist;
    for (int i = 0; i < (*cloud_filtered).size(); i++)
    {
        pcl::PointXYZ pt(cloud_filtered->points[i].x, cloud_filtered->points[i].y, cloud_filtered->points[i].z);
        if ((pow(pt.x, 2) + pow(pt.y, 2) + pow(pt.z, 2)) > 6.7) // e.g. remove all pts below zAvg
        {
            inliers_dist->indices.push_back(i);
        }
    }
    extract_dist.setInputCloud(cloud_filtered);
    extract_dist.setIndices(inliers_dist);
    extract_dist.setNegative(true);
    extract_dist.filter(*cloud_filtered);
    writer.write<pcl::PointXYZ> ("/home/b3mylq/ROS_ws/pcl_ws/src/depth_make/data/hex_filtered.pcd", *cloud_filtered, false);
    // vg.setInputCloud (cloud);
    // vg.setLeafSize (0.01f, 0.02f, 0.02f);
...
int main (int argc, char **argv){
    ros::init (argc, argv, "echo_cluster");
	ros::NodeHandle nh;

    pub_cluster = nh.advertise<pcl::PointCloud<pcl::PointXYZ>>("cluster_pcl_points", 100, true);
    ros::Subscriber pointCLoudSub = nh.subscribe<sensor_msgs::PointCloud2>("/velodyne_points", 100, ClusterCallback);
